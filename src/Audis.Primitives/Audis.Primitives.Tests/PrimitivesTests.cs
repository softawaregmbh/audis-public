...
public class PrimitivesTests
{
    ...
    public void TestKnowledgeIdentifierInvalidFormat()
    {
        ...
        Assert.That(ex.Message,
            Is.EqualTo(
                "The KnowledgeIdentifier has an invalid format: \"no-leading-hashtag\", Expected starting with #."));
    }
    ...
    public void TestQuestionIdInvalidFormat(string input, bool throws)
    {
        if (throws)
        {
            ...
            Assert.That(ex.Message,
                Is.EqualTo(
                    $"The QuestionId has an invalid format: \"{input}\", Expected \"<question-catalog-name>:<lineNumber>\"."));
        }
        else
        {
    ...
    public void TestQuestionIdQuestionCatalogAndLineNumberConstructor()
    {
        ...
        Assert.That(questionId.QuestionCatalogName,
            Is.EqualTo(new QuestionCatalogName(questionId.Value.Split(':')[0])));
    ...
    public void TestAnswerIdInvalidFormat(string input, bool throws)
    {
        if (throws)
        {
            ...
            Assert.That(ex.Message,
                Is.EqualTo(
                    $"The AnswerId has an invalid format: \"{input}\", Expected \"<question-catalog-name>:<questionLineNumber>/<answerLineNumber>\"."));
        }
        else
        {
